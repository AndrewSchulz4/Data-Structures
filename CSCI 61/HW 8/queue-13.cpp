...
void line_simulation(Average& avg, ofstream& output, int& count, int& officehour);
string topic();
string nombre();
void sortfile(ifstream& in, string column, string direction, int count);
void find(string column, string name, string namea [], string topic [], int officehour [], int count);
...
int main()
{
  ...
  for(int i = 0; i < 100; i++)
  {
    line_simulation(avg, output, count, officehour);
    officehour++;
  }
  output.close();
...
void line_simulation(Average& avg, ofstream& output, int& count, int& officehour)
{
  ...
  for(int i = 1; i <= 60; i++)
  {
    ...
    if(prob_at_time > .9)
    {
      output << officehour << "\t" << name << "\t" << top << endl;
      count++;
...
//read in values to an ARRAY
  ...
  if(direction == "descending")
  {
    ...
    {
      newout << namearr[i] << "\t" << officeh[i] << "\t" << topicarr[i] << endl;
    }
  }
  else
  {
  ...
  string SoT;
  string name;
  cout << "would you like to find a student or topic? ";
  cin >> SoT;
  cout << "Name/topic to search : ";
  cin >> name;
  find(SoT, name, namearr, topicarr, officeh, count);
}
...
void find(string column, string name, string namea [], string topic [], int officehour [], int count)
{
  ...
  if(column == "topic")
  {
    cout << "Came to Office Hours for " << topic  << ":"<< endl;
    for(int i = 0; i < count; i++)
    {
      if(topic[i] == name)
      {
        cout << "\t" << namea[i] << " at office hour: " << officehour[i] << endl;
        found = true;
      }
  ...
  {
    cout << name << " came to Office Hour for " << ":"<< endl;
    for(int i = 0; i < count; i++)
    {
      if(namea[i] == name)
      {
        cout << "\t" << topic[i] << " at office hour: " << officehour[i] << endl;
        found = true;
      }
    }
  }
  if(!found)
  {
    cout << "Name or Topic not found" << endl;
  }
}
