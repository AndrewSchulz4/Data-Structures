...
namespace main_savitch_11
{
    ...
    public:
        ...
        set( );//Works
        set(const set& source);//Works
        ~set( ) { clear( ); }
        // MODIFICATION MEMBER FUNCTIONS
        void operator =(const set& source);//Works
        void clear();
        bool insert(const Item& entry);//Works
        std::size_t erase(const Item& target);
        // CONSTANT MEMBER FUNCTIONS
        std::size_t count(const Item& target) const;//Works
        bool empty( ) const { return (data_count == 0); }
        // SUGGESTED FUNCTION FOR DEBUGGING
        void print(int indent) const;//Works
    private:
        // MEMBER CONSTANTS
        ...
        bool loose_insert(const Item& entry);//Works
        bool loose_erase(const Item& target);
        void remove_biggest(Item& removed_entry);
        void fix_excess(std::size_t i);//Works
