...
namespace main_savitch_15
{
    ...
    template <class Item>
    std::size_t graph<Item>::longest_path(std::size_t start, std::size_t number)
    {
      std::size_t largest = 0;
      if(size() > 2)
      {
        //will store all possible edges/vertices in answer
        std::set<std::size_t> answer;

        //goes through and finds all indexes with edges to others
        for (std::size_t index = 0; index < many_vertices; index++)
        {
            if (edges[start][index])
                answer.insert(index);
        }
        //iterates through the vector, finding the longest path for each vertex
        for(auto i = answer.begin() ; i != answer.end(); i++)
        {
          if(1 + longest_path(*i, number + 1) > largest)
          {
            //changes largest based off of if set position has longer path
            largest = 1 + longest_path(*i, number + 1);
          }
        }
      }
      else
